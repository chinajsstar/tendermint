...
- [rpc]: changed the output format for the `/status` endpoint (see https://godoc.org/github.com/chinajsstar/tendermint/rpc/core#Status)
...
IMPROVEMENTS:
- rpc/client: can act as event bus subscriber (See https://github.com/chinajsstar/tendermint/issues/945).
...
- Config and Flags:
  - The `config` map is replaced with a [`Config` struct](https://github.com/chinajsstar/tendermint/blob/master/config/config.go#L11),
...
- Merge `tendermint/go-p2p -> chinajsstar/tendermint/p2p` and `tendermint/go-rpc -> chinajsstar/tendermint/rpc/lib`
...
- Docker image improved, slimmed down (using Alpine), and changed from tendermint/tmbase to chinajsstar/tendermint
