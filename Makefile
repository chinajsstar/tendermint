...
BUILD_FLAGS = -ldflags "-X github.com/chinajsstar/tendermint/version.GitCommit=`git rev-parse --short=8 HEAD`"
...
draw_deps:
	...
	@goviz -i github.com/chinajsstar/tendermint/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
...
build-docker:
	...
	docker build --label=tendermint --tag="chinajsstar/tendermint" DOCKER
